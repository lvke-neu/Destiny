...
namespace Destiny
{
	TorusComponent::TorusComponent()
	{
		...
		auto mesh = MeshProvider::Create_Torus_PositionNormalTexcoord();
		mesh->load();
	...
	void TorusComponent::onUpdate(float deltaTime)
	{
	}

	RTTR_REGISTRATION
	{
		rttr::registration::class_<TorusComponent>("TorusComponent")
			.constructor<>();
	}
}