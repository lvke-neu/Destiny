#pragma once
#include "RigidBodyComponent.h"

class btRigidBody;
class btMotionState;
class btCollisionShape;
namespace Destiny
{
	class RigidBodyBoxVisualComponent;
	class RigidBodyBoxComponent : public RigidBodyComponent, public std::enable_shared_from_this<RigidBodyBoxComponent>
	{
		RTTR_ENABLE(RigidBodyComponent);
	public:
		RigidBodyBoxComponent();
		virtual ~RigidBodyBoxComponent();
	public:
		virtual btRigidBody* getRigidBody() override;

		virtual void onAddToNode() override;
		virtual void onNodeTransformChanged() override;
		virtual void onEnterScene() override;
		virtual void onLeaveScene() override;
	public:
		GET(float, restitution);
		void set_restitution(float restitution);
		GET(float, mass);
		void set_mass(float mass);
		GET(float, friction);
		void set_friction(float friction);
		GET(DirectX::XMFLOAT3, boxHalfExtents);
		void set_boxHalfExtents(DirectX::XMFLOAT3 boxHalfExtents);
	private:
		void updateRigidBodyTransform();
	private:
		btRigidBody* m_btRigidBody;
		btMotionState* m_btMotionState;
		btCollisionShape* m_btCollisionShape;
		
		float m_restitution;
		float m_friction;
		float m_mass;
		DirectX::XMFLOAT3 m_boxHalfExtents;

		std::shared_ptr<RigidBodyBoxVisualComponent> m_rigidBodyBoxVisualComponent;
	};

	btRigidBody* RigidBodyBoxComponent::getRigidBody()
	{
		return m_btRigidBody;
	}
}