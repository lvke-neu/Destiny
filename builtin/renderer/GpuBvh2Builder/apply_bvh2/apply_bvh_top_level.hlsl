...
AppendStructuredBuffer<uint> nextLayerBuffer : register(u2);
...
void BVHNodeIntersect(uint rootNodeIndex)
{
    ...
    while (stackPtr > 0)
    {
        ...
        if (AABBFrustumIntersect_Planes(box, g_view, g_proj))
        {
            if (IsLeaf(flag))
            {
                nextLayerBuffer.Append(nodeIndex); // 写叶子节点本身
            }
            else if (depth + 1 >= MaxBatchDepth)
            {
                nextLayerBuffer.Append(left);
                nextLayerBuffer.Append(right);
            }
            else
            {
                StackNode snLeft;
