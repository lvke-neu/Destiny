...
namespace Destiny
{
	...
	std::shared_ptr<GeometryShader> GraphicsSystem::compileGeometryShader(const char* path)
	{
		std::string tmpPath = path;
		size_t prefix = tmpPath.find("assets://");
		if (prefix == std::string::npos)
		...
		ID3DBlob* ppBlobOut = nullptr;
		ID3DBlob* errorBlob = nullptr;
		HRESULT hr = D3DCompileFromFile(Utility::MultiByte2WideChar(tmpPath).c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, "GS", "gs_5_0",
			D3DCOMPILE_ENABLE_STRICTNESS, 0, &ppBlobOut, &errorBlob);
		...
		if (FAILED(hr))
		{
			if (errorBlob != nullptr)
			{
				LOG_ERROR("CompileGeometryShader Failed:" + reinterpret_cast<const char*>(errorBlob->GetBufferPointer()));
			}
			SAFE_RELEASE(errorBlob);
			return nullptr;
		}
