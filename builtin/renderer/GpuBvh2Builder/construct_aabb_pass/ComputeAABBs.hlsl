...
RWStructuredBuffer<BoundingBox> boundingBoxBuffer : register(u5);
RWStructuredBuffer<uint> bitonicSortIndicesBuffer : register(u6);
...
BoundingBox ComputeLeafAABB(uint leafIndex, uint unused, out uint2 flags)
{
    ...
    //uint2 unusedFlag;
    //AABB box = BoundingBoxToAABB(BVHReadBoundingBox(metadata.instanceDesc.AccelerationStructure, bottomLevelAccelerationStructure, 0, unusedFlag));
    ...
    uint index = bitonicSortIndicesBuffer[leafIndex];
    flags.x = leafIndex | IsLeafFlag;
    flags.y = 1;
    return boundingBoxBuffer[index];
}
