...
RWStructuredBuffer<Cluster64> ClustersSmallers : register(u0);
RWStructuredBuffer<uint> CommitClusters : register(u1);
RWStructuredBuffer<AABB> AABBBuffer : register(u2);
RWStructuredBuffer<uint> ClusterOffsets : register(u3);
RWStructuredBuffer<uint> ClusterElements : register(u4);
AppendStructuredBuffer<uint> CommitElements : register(u5);
...
{
    uint idx = CommitClusters[dispatchThreadID.x];
    Cluster64 cluster64 = ClustersSmallers[idx];
    
    uint clusterOffset = ClusterOffsets[cluster64.clusterIndex];

    for (uint i = cluster64.clusterStart; i < cluster64.clusterCount; i++)
    {
        uint clusterElement = ClusterElements[clusterOffset + i];

        AABB aabb = AABBBuffer[clusterElement];

        BoundingBox box;
        box.center = (aabb.min + aabb.max) * 0.5f;
        box.halfDim = (aabb.max - aabb.min) * 0.5f;

        if (AABBFrustumIntersect_Planes(box, g_view, g_proj))
        {
            CommitElements.Append(clusterElement);
        }
    }
}                                                                  