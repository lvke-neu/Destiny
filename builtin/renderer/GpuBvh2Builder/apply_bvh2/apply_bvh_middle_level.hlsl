...
static const uint MaxBatchDepth = 4;
...
RWStructuredBuffer<StackNode> stack : register(u1);
RWStructuredBuffer<uint> nextLayerBuffer : register(u2);
AppendStructuredBuffer<uint> nextLayerBuffer2 : register(u3);
...
void BVHNodeIntersect(uint rootNodeIndex)
{
    ...
    while (stackPtr > 0)
    {
        ...
        uint nodeIndex = sn.nodeIndex;
        uint depth = sn.depth;

        uint2 flag;
        BoundingBox box = GetBoxFromBuffer(outputBVH, SizeOfBVHOffsets, nodeIndex, flag);
        ...
        if (AABBFrustumIntersect_Planes(box, g_view, g_proj))
        {
            if (IsLeaf(flag))
            {
                nextLayerBuffer2.Append(nodeIndex); // 写叶子节点本身
            }
            else if (depth + 1 >= MaxBatchDepth)
            {
                nextLayerBuffer2.Append(left);
                nextLayerBuffer2.Append(right);
            }
            else
            {
                StackNode snLeft;
                snLeft.nodeIndex = left;
                snLeft.depth = depth + 1;
                stack[stackPtr++] = snLeft;

                StackNode snRight;
                snRight.nodeIndex = right;
                snRight.depth = depth + 1;
                stack[stackPtr++] = snRight;
            }
        }
    }
}
...
{
    BVHNodeIntersect(nextLayerBuffer[dispatchThreadID.x]);
}                                                                  