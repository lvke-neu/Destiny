...
AppendStructuredBuffer<uint> CommitClusters : register(u1);
...
void CS(uint3 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    BoundingBox box;
    box.center = (cluster64.aabb.min + cluster64.aabb.max) * 0.5f;
    box.halfDim = (cluster64.aabb.max - cluster64.aabb.min) * 0.5f; 

    if (AABBFrustumIntersect_Planes(box, g_view, g_proj))
    {
        CommitClusters.Append(dispatchThreadID.x);
    }
}                                                                  