...
namespace Destiny
{
	...
	{
		const unsigned char* curSrc = (const unsigned char*)src;
		...
		while (len >= 3)
		{
			...
			curDst += 4;
			len -= 3;
			if (0 == (curSrc - (const unsigned char*)src) % 57)
			{
				*curDst++ = '\n';
			}
		}
	...
	{
		unsigned char* curDst = (unsigned char*)dst;
		...
		for (size_t i = 0; i < srcLen; ++i)
		{
			...
			ch = s_base64DecodeTable[unsigned char(ch)];
			if (ch < 0)//char == -1
			{
				continue;
			}
		...
		return (curDst - (unsigned char*)dst);
	}
