...
float4 PS(VertexOut pIn) : SV_Target
{
	...
	float metallic = t_metallic.Sample(s_sampler, pIn.texcoord).b;
	float roughness = t_roughness.Sample(s_sampler, pIn.texcoord).g;
	float ao = t_ao.Sample(s_sampler, pIn.texcoord).r;
